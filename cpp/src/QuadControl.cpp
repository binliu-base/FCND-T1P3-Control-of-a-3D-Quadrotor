...
VehicleCommand QuadControl::GenerateMotorCommands(float collThrustCmd, V3F momentCmd)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
	...
	float f1 = (collThrustCmd + tor_x / l + tor_y / l + tor_z ) / 4.f;   // front left
	float f2 = (collThrustCmd - tor_x / l + tor_y / l - tor_z ) / 4.f;   // front right
	float f3 = (collThrustCmd - tor_x / l - tor_y / l + tor_z) / 4.f;    // rear left 
	float f4 = (collThrustCmd + tor_x / l - tor_y / l - tor_z) / 4.f;    // rear right
...
V3F QuadControl::RollPitchControl(V3F accelCmd, Quaternion<float> attitude, float collThrustCmd)
{
	...
	////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
	float b_x = R(0, 2);
	float b_x_c = accelCmd[0];
	float b_x_dot_c = kpBank * (b_x_c - b_x);

	float b_y = R(1, 2);
	float b_y_c = accelCmd[1];
	...
	V3F b_dot_Cmd;
	b_dot_Cmd.x = b_x_dot_c;
	b_dot_Cmd.y = b_y_dot_c;
	b_dot_Cmd.z = 0;
